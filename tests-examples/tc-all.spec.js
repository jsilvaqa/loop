...
// test('Test Case 1 - Assert Implement User Auth', async ({browser}) =>
    ...
    const todoColumnW = page.locator('text=To Do').locator('#root > div > div.flex-1.flex.flex-col.bg-gray-100 > main > div > div > div:nth-child(1)');
    // Verify the task exists in "To Do"
    const taska = todoColumnW.locator('text=Implement user authentication');
...
// {
    ...
    const todoColumnW = page.locator('text=To Do').locator('#root > div > div.flex-1.flex.flex-col.bg-gray-100 > main > div > div > div:nth-child(1)');
    // Verify the task exists in "To Do"
    const task = todoColumnW.locator('text=Fix navigation bug');
...
// {
    ...
    const inProgressW = page.locator('text=In Progress').locator('#root > div > div.flex-1.flex.flex-col.bg-gray-100 > main > div > div > div:nth-child(2)');
    // Verify the task exists in "In Progress"
    const task = inProgressW.locator('text=Design system updates');
    await expect(task).toBeVisible();
    //Verify tags
...
// {
    ...
    // Locate the "To Do" column
    const todoColumnM = page.locator('text=In Progress').locator('#root > div > div.flex-1.flex.flex-col.bg-gray-100 > main > div > div > div:nth-child(1)');
    // Verify the task exists in "To Do"
    const task = todoColumnM.locator('text=Push notification system');
    await expect(task).toBeVisible();
    //Verify tags
    await expect(task.locator('text=Feature')).toBeVisible();
...
// {
    // Locate the "In Progress" column
    const inProgressM = page.locator('text=In Progress').locator('#root > div > div.flex-1.flex.flex-col.bg-gray-100 > main > div > div > div:nth-child(1)');
    // Verify the task exists in "In Progress"
    const task = inProgressM.locator('text=Offline mode');
    await expect(task).toBeVisible();
    //Verify tags
    await expect(task.locator('text=Feature')).toBeVisible();
    await expect(task.locator('text=High Priority')).toBeVisible();

// test('Test Case 5 - Assert App Icon Design', async ({browser}) =>
// {
// Locate the "Done" column
    ...
    // Verify the task exists in "Done"
